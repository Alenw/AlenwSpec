...
+ (instancetype)initWithCustomView:(UIView *)customview target:(id)target action:(SEL)action{
    if ([customview isKindOfClass:[UIControl class]]) {
        UIControl *ctr=(UIControl *)customview;
        [ctr addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    }
    return [[SKYBarButtonItem alloc]initWithCustomView:customview];
}
